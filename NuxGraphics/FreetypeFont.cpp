  ...
  //      return;  // NULL string; return
    ...
    for(int i=0; i< num_cararcter; i++)
    {
        ...
        if(texture_id != get_null_character_texture_id())
        {
            ...
            glBegin(GL_QUADS);
            glTexCoord2f(0.0f, 0.0f); glVertex3i(bitmap_x,  bitmap_y, 1);
            glTexCoord2f(0.0f, 1.0f); glVertex3i(bitmap_x,  bitmap_y + height, 1);
            glTexCoord2f(1.0f, 1.0f); glVertex3i(bitmap_x + width,  bitmap_y + height, 1);
            glTexCoord2f(1.0f, 0.0f); glVertex3i(bitmap_x + width,  bitmap_y, 1);
            glEnd();
