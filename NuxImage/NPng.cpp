...
{\
    nuxAssertMsg(0, "[PNG Read Error] Cannot open %s for read.", FileNameAnsi); \
    return 0;\
}
#if defined(_WIN32)
...
#define PNG_READHEADER
  ...
  NBitmapData *read_png_rgba(const TCHAR *filename)
  {
    ...
    if (!FileNameAnsi)
    {
      nuxAssertMsg(0, "[read_png_rgba] Incorrect file name: %s.", filename);
      return 0;
    }
...
#endif
    {
      nuxAssert("[read_png_rgba] File not found: %s.");
      return 0;
    }
    ...
    if (r == 0)
    {
      nuxAssertMsg(0, "[read_png_rgba] Png read error.");
    }
    ...
    if ( channels == 3 )
    {
      //nuxAssertMsg(0, "[read_png_rgba] Unable to open image of type RGB %s using read_png_rgba().", filename);
      //nuxAssertMsg(0, "[read_png_rgba] Try using read_png_rgb() instead!");
      if (fp) fclose(fp);
  ...
  NBitmapData *read_png_rgb(const TCHAR *filename)
  {
    ...
    if (!FileNameAnsi)
    {
      nuxAssertMsg(0, "[read_png_rgb] Incorrect file name: %s.", filename);
      return 0;
    }
...
#endif
    {
      nuxAssert("[read_png_rgb] File not found: %s.");
      return 0;
    }
    ...
    if ( channels == 4 )
    {
      //nuxAssertMsg(0, "[read_png_rgba] Unable to open image of type RGBA %s using read_png_rgb().", filename);
      //nuxAssertMsg(0, "[read_png_rgba] Try using read_png_rgba() instead!");
      if (fp) fclose(fp);
