...
namespace nux
{
  ...
  */
  ...
  NBitmapData *read_tga_file(const TCHAR *file_name)
  {
    ...
    {
      //nuxAssertMsg(0, "[read_tga_file] File not found: %s", file_name);
      return 0;
    }
    ...
    {
      //nuxAssertMsg(0, "[read_tga_file] Can't open file: %s", file_name);
      return 0;
    }
    ...
    {
      fileStream.close();
      //nuxAssertMsg(0, "[read_tga_file] Error while reading the TGA footer: %s", file_name);
      return 0;
    }
    ...
    if (!footer_present)
    {
      ...
      //nuxAssertMsg(0, "[read_tga_file] This is not a TGA 2.0 file: %s", file_name);
      fileStream.close();
      return 0;
    }
    ...
    {
      fileStream.close();
      //nuxAssertMsg(0, "[read_tga_file] Error while reading the TGA header: %s", file_name);
      return 0;
    }
    ...
    switch(header.image_type)
    {
      ...
      default:
        fileStream.close();
        //nuxAssertMsg(0, "[read_tga_file] Couldn't find the image type: %s", file_name);
        return 0;
    }
    ...
    if (((header.image_type == TGA_COLORMAP) || (header.image_type == TGA_RLE_COLORMAP)) && header.colormap_type == 0)
    {
      ...
      // problem in file
      fileStream.close();
      //nuxAssertMsg(0, "[read_tga_file] Couldn't find the colormap: %s", file_name);
      return 0;
    }
    ...
    if (header.image_type == TGA_COLORMAP || header.image_type == TGA_RLE_COLORMAP)
    {
      // Not supported
      fileStream.close();
      //nuxAssertMsg(0, "[read_tga_file] TGA colormap image is not supported: %s", file_name);
      return 0;
    }
  ...
  HReport write_tga_file(const TCHAR *file_name, NBitmapData *image)
  {
    ...
    if (file_name == 0)
    {
      nuxAssertMsg(0, "[write_tga_file] Incorrect file name.");
