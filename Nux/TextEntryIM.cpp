...
namespace nux
{
  ...
  NUX_IMPLEMENT_OBJECT_TYPE(TextEntryIM);

  TextEntryIM::TextEntryIM(const char* text, NUX_FILE_LINE_DECL)
    ...
    , key_nav_mode_(false)
  {
    ...
    mouse_down.connect(sigc::mem_fun(this, &TextEntryIM::RecvMouseDown));
    mouse_drag.connect(sigc::mem_fun(this, &TextEntryIM::RecvMouseDrag));
    mouse_up.connect(sigc::mem_fun(this, &TextEntryIM::RecvMouseUp));
    mouse_double_click.connect(sigc::mem_fun(this, &TextEntryIM::RecvMouseDoubleClick));
    mouse_enter.connect(sigc::mem_fun(this, &TextEntryIM::RecvMouseEnter));
    mouse_leave.connect(sigc::mem_fun(this, &TextEntryIM::RecvMouseLeave));

    key_down.connect(sigc::mem_fun(this, &TextEntryIM::RecvKeyEvent));

    begin_key_focus.connect(sigc::mem_fun(this, &TextEntryIM::RecvStartKeyFocus));
    end_key_focus.connect(sigc::mem_fun(this, &TextEntryIM::RecvEndKeyFocus));
  ...
  long TextEntryIM::PostLayoutManagement(long layoutResult)
  {
  ...
  void TextEntryIM::GeometryChanged()
  {
  ...
  Area* TextEntryIM::FindAreaUnderMouse(const Point& mouse_position, NuxEventType event_type)
  {
  ...
  void TextEntryIM::ProcessMouseEvent(int event_type, int x, int y, int dx, int dy, unsigned long button_flags, unsigned long key_flags)
  {
  ...
  void TextEntryIM::ProcessKeyEvent(
...
//       }
  ...
  void TextEntryIM::RecvMouseDoubleClick(int x, int y, unsigned long button_flags, unsigned long key_flags)
  {
    ProcessMouseEvent(NUX_MOUSE_DOUBLECLICK, x, y, 0, 0, button_flags, key_flags);
  }

  void TextEntryIM::RecvMouseUp(int x, int y, unsigned long button_flags, unsigned long key_flags)
  {
    ProcessMouseEvent(NUX_MOUSE_RELEASED, x, y, 0, 0, button_flags, key_flags);
  }

  void TextEntryIM::RecvMouseDown(int x, int y, unsigned long button_flags, unsigned long key_flags)
  {
    ProcessMouseEvent(NUX_MOUSE_PRESSED, x, y, 0, 0, button_flags, key_flags);
  }

  void TextEntryIM::RecvMouseDrag(int x, int y, int dx, int dy, unsigned long button_flags, unsigned long key_flags)
  {
    ProcessMouseEvent(NUX_MOUSE_MOVE, x, y, dx, dy, button_flags, key_flags);
  }
  
  void TextEntryIM::RecvMouseEnter(int x, int y, unsigned long button_flags, unsigned long key_flags)
  {
#if defined(NUX_OS_LINUX)
  ...
  void TextEntryIM::RecvMouseLeave(int x, int y, unsigned long button_flags, unsigned long key_flags)
  {
#if defined(NUX_OS_LINUX)
    if (caret_cursor_ != None)
    {
  ...
  void TextEntryIM::RecvStartKeyFocus()
  {
    key_nav_mode_           = true;
    text_input_mode_        = false;
  ...
  void TextEntryIM::RecvEndKeyFocus()
  {
    key_nav_mode_     = false;
    text_input_mode_  = false;
  ...
  void TextEntryIM::Draw(GraphicsEngine& gfxContext, bool forceDraw)
  {
    MainDraw();
  ...
  void TextEntryIM::DrawContent(GraphicsEngine& gfxContext, bool forceDraw)
  {
    //MainDraw();
  }

  void TextEntryIM::PostDraw(GraphicsEngine& gfxContext, bool forceDraw)
  {
  ...
  std::string const& TextEntryIM::GetCompletion() const
  {
    return completion_;
  }

  void TextEntryIM::SetCompletionColor(const Color &color)
  {
    completion_color_ = color;
    QueueRefresh(true, true);
  }

  Color const& TextEntryIM::GetCompletionColor() const
  {
    return completion_color_;
  }

  void TextEntryIM::SetTextColor(const Color &text_color)
  {
  ...
  Color const& TextEntryIM::GetTextColor() const
  {
    return _text_color;
  }
...
//       background_->Draw(canvas, 0, 0, GetBaseWidth, GetBaseHeight);
  ...
  CairoGraphics* TextEntryIM::EnsureCanvas()
  {
    if (canvas_)
    {
      ...
      {
        nuxDebugMsg("[TextEntryIM::EnsureCanvas] Recreate canvas");
        delete canvas_;
        canvas_ = NULL;
      }
    }
  ...
  void TextEntryIM::QueueRefresh(bool relayout, bool adjust_scroll)
  {
    if (relayout)
      ResetLayout();
  ...
  void TextEntryIM::ResetImContext()
  {
    if (need_im_reset_)
    {
      need_im_reset_ = false;
...
//         gtk_im_context_reset(im_context_);
  ...
  void TextEntryIM::DrawText(CairoGraphics *canvas)
  {
    PangoLayout *layout = EnsureLayout();
  ...
  bool TextEntryIM::CursorBlinkCallback(TextEntryIM *self)
  {
  ...
  void TextEntryIM::QueueCursorBlink()
  {
    if (!cursor_blink_timer_)
  ...
  void TextEntryIM::GetCursorRects(Rect *strong, Rect *weak)
  {
    int strong_x, strong_y, strong_height;
    int weak_x, weak_y, weak_height;
    GetCursorLocationInLayout(&strong_x, &strong_y, &strong_height,
  ...
  void TextEntryIM::UpdateCursorRegion()
  {
  ...
  void TextEntryIM::DrawCursor(CairoGraphics *canvas)
  {
    if (!cursor_visible_)
      return;
  ...
  Color TextEntryIM::GetSelectionBackgroundColor()
  {
    return kDefaultSelectionBackgroundColor;
  }

  Color TextEntryIM::GetSelectionTextColor()
  {
    return kDefaultSelectionTextColor;
  }

  void TextEntryIM::GetCursorLocationInLayout(int *strong_x, int *strong_y,
    int *strong_height,
  ...
  PangoLayout* TextEntryIM::EnsureLayout()
  {
    if (!cached_layout_)
    {
  ...
  void TextEntryIM::QueueTextDraw()
  {
  ...
  PangoLayout* TextEntryIM::CreateLayout()
  {
...
//           dir = PANGO_DIRECTION_LTR;
  ...
  int TextEntryIM::TextIndexToLayoutIndex(int text_index, bool consider_preedit_cursor)
  {
      if (visible_)
      {
  ...
  int TextEntryIM::LayoutIndexToTextIndex(int layout_index)
  {
    if (visible_)
    {
  ...
  int TextEntryIM::GetCharLength(int index)
  {
    const char *text = text_.c_str();
  ...
  int TextEntryIM::GetPrevCharLength(int index)
  {
    const char *text = text_.c_str();
    const char *ptr = text + index;
  ...
  void TextEntryIM::EnterText(const char *str)
  {
  ...
  void TextEntryIM::Select(int start, int end) {
    int text_length = static_cast<int>(text_.length());
    if (start == -1)
      start = text_length;
    if (end == -1)
  ...
  void TextEntryIM::SelectAll() {
    SetSelectionBounds(0, static_cast<int>(text_.length()));
    QueueRefresh(false, true);
  }

  CairoGraphics::Alignment TextEntryIM::GetAlign() const
  {
    return align_;
  }

  void TextEntryIM::SetAlign(CairoGraphics::Alignment align)
  {
    align_ = align;
    QueueRefresh(true, true);
  }

  bool TextEntryIM::im_active()
  {
    return ime_active_;
  }

  void TextEntryIM::DeleteSelection()
  {
    int start, end;
  ...
  void TextEntryIM::CopyClipboard()
  {
//     int start, end;
//     if (GetSelectionBounds(&start, &end))
//     {
...
//     }
  ...
  void TextEntryIM::CutClipboard()
  {
    CopyClipboard();
    DeleteSelection();
  }

  void TextEntryIM::PasteClipboard()
  {
//     GtkWidget *widget = GetWidgetAndCursorLocation(NULL);
//     if (widget)
//     {
...
//     }
  ...
  void TextEntryIM::BackSpace(MovementStep step)
  {
    if (GetSelectionBounds(NULL, NULL))
    {
      DeleteSelection();
  ...
  void TextEntryIM::Delete(MovementStep step)
  {
    if (GetSelectionBounds(NULL, NULL))
    {
      DeleteSelection();
  ...
  void TextEntryIM::ToggleOverwrite()
  {
    overwrite_ = !overwrite_;
  }

  void TextEntryIM::UpdateSelectionRegion()
  {
  ...
  int TextEntryIM::MoveVisually(int current_index, int count)
  {
  ...
  int TextEntryIM::MoveWords(int current_index, int count)
  {
    nuxAssert(current_index >= 0 &&
      current_index <= static_cast<int>(text_.length()));
    nuxAssert(count);
  ...
  int TextEntryIM::MoveDisplayLines(int current_index, int count)
  {
    nuxAssert(current_index >= 0 &&
      current_index <= static_cast<int>(text_.length()));
    nuxAssert(count);
  ...
  int TextEntryIM::MovePages(int current_index, int count)
  {
    nuxAssert(current_index >= 0 &&
      current_index <= static_cast<int>(text_.length()));
    nuxAssert(count);
  ...
  int TextEntryIM::MoveLineEnds(int current_index, int count)
  {
    nuxAssert(current_index >= 0 &&
      current_index <= static_cast<int>(text_.length()));
    nuxAssert(count);
...
// #endif
  ...
  void TextEntryIM::SetCursor(int cursor)
  {
    if (cursor != cursor_)
    {
      ResetImContext();
  ...
  int TextEntryIM::XYToTextIndex(int x, int y)
  {
    int width, height;
    PangoLayout *layout = EnsureLayout();
    const char *text = pango_layout_get_text(layout);
  ...
  void TextEntryIM::SetSelectionBounds(int selection_bound, int cursor)
  {
  ...
  void TextEntryIM::SetFontFamily(const char *font)
  {
    font_family_ = font;
    QueueRefresh(true, true);
  }

  void TextEntryIM::SetFontSize(double font_size)
  {
    font_size_ = font_size;
    QueueRefresh(true, true);
  }

  void TextEntryIM::SetFontOptions(const cairo_font_options_t *options)
  {
  ...
  bool TextEntryIM::InspectKeyEvent(unsigned int eventType,
    ...
    const char* character)
  {
    ...
    {
      // Enable to exit the TextEntryIM when in write mode(hack for unity dash)
