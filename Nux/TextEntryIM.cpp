...
namespace nux
{
    ...
    , cursor_index_in_layout_(-1)
  {
    ...
    cursor_blink_signal_->time_expires.connect(sigc::mem_fun(this, &TextEntryIM::CursorBlinkCallback));
...
//       background_->Draw(canvas, 0, 0, GetBaseWidth, GetBaseHeight);
  ...
  void TextEntryIM::StartKeyboardFocus()
  {
  ...
  void TextEntryIM::EndKeyboardFocus()
  {
    if (has_keyboard_focused_)
    {
      has_keyboard_focused_ = false;
  ...
  void TextEntryIM::AdjustScroll(AdjustScrollPolicy policy)
  {
    if (policy == NO_SCROLL)
      return;
  ...
  void TextEntryIM::QueueRefresh(bool relayout, AdjustScrollPolicy policy)
  {
    if (relayout)
      ResetLayout();
...
//         gtk_im_context_reset(im_context_);
  ...
  void TextEntryIM::UpdateContentRegion()
  {
  ...
  void TextEntryIM::CursorBlinkCallback(void *data)
  {
  ...
  void TextEntryIM::StartCursorBlink()
  {
  ...
  void TextEntryIM::StopCursorBlink()
  {
    if (cursor_blink_handle_.IsValid())
    {
  ...
  void TextEntryIM::GetCursorLocationInLayout(PangoRectangle *strong, PangoRectangle *weak)
  {
...
//           dir = PANGO_DIRECTION_LTR;
  ...
  int TextEntryIM::MoveLogically(int current_index, int count)
  {
    nuxAssert(current_index >= 0 && current_index <= static_cast<int>(text_.length()));
    nuxAssert(count);
