...
#include "Basic/Math/MathUtility.h"
...
//
bool TextDocument::init(const TCHAR *filename)
{
...
//
    ...
    bool TextDocument::init(int hFile)
#else
...
#endif
{
	...
	ReadFile(hFile, buffer, m_nDocLength_bytes, (DWORD*)&numread, 0);
...
bool TextDocument::EmptyDoc()
{
	...
	m_pLineBuf_byte = new inl::t_u32[0x1000];
	m_pLineBuf_char = new inl::t_u32[0x1000];
...
//
int TextDocument::getchar(inl::t_u32 offset, inl::t_u32 lenbytes, inl::t_u32 *pch32)
{
//	BYTE	*rawdata   = (BYTE *)(buffer + offset + m_nHeaderSize);
	BYTE	rawdata[16];

	lenbytes = inl::Min<inl::t_u32>(16, lenbytes);
	...
	*pch32 = (inl::t_u32)(BYTE)rawdata[0];
	return 1;
...
//
inl::t_u32 TextDocument::gettext(inl::t_u32 offset, inl::t_u32 lenbytes, TCHAR *buf, inl::t_u32 *buflen)
{
//	BYTE	*rawdata = (BYTE *)(buffer + offset + m_nHeaderSize);

	inl::t_u32 chars_copied = 0;
	inl::t_u32 bytes_processed = 0;
	...
	{
		BYTE   rawdata[0x100];
		size_t rawlen = inl::Min<inl::t_u32>(lenbytes, 0x100);
        ...
        // convert to UTF-8
        rawlen = rawdata_to_utf8(rawdata, rawlen, (BYTE*)buf, &tmplen);
...
inl::t_u32 TextDocument::getdata(inl::t_u32 offset, BYTE *buf, size_t len)
{
...
{
	inl::t_u32 offset_bytes		= 0;
	inl::t_u32 offset_chars		= 0;
	inl::t_u32 linestart_bytes	= 0;
	inl::t_u32 linestart_chars	= 0;
	inl::t_u32 bytes_left	    = m_nDocLength_bytes - m_nHeaderSize;

	inl::t_u32 buflen  = m_nDocLength_bytes - m_nHeaderSize;

	// allocate the line-buffer for storing each line's BYTE offset
	if((m_pLineBuf_byte = new inl::t_u32[buflen+1]) == 0)
		return false;

	// allocate the line-buffer for storing each line's CHARACTER offset
	if((m_pLineBuf_char = new inl::t_u32[buflen+1]) == 0)
		return false;
	...
	for(offset_bytes = 0; offset_bytes < buflen; )
	{
		...
		inl::t_u32 len = getchar(offset_bytes, buflen - offset_bytes, &ch32);
		offset_bytes += len;
		offset_chars += 1;
...
//
inl::t_u32 TextDocument::linecount()
{
	return m_nNumLines;
}
...
//
inl::t_u32 TextDocument::longestline(int tabwidth)
{
	//inl::t_u32 i;
	inl::t_u32 longest = 0;
	inl::t_u32 xpos = 0;
...
//
bool TextDocument::lineinfo_from_lineno(inl::t_u32 lineno, inl::t_u32 *lineoff_chars,  inl::t_u32 *linelen_chars, inl::t_u32 *lineoff_bytes, inl::t_u32 *linelen_bytes)
{
...
//
bool TextDocument::lineinfo_from_offset(inl::t_u32 offset_chars, inl::t_u32 *lineno, inl::t_u32 *lineoff_chars, inl::t_u32 *linelen_chars, inl::t_u32 *lineoff_bytes, inl::t_u32 *linelen_bytes)
{
	inl::t_u32 low  = 0;
	inl::t_u32 high = m_nNumLines-1;
	inl::t_u32 line = 0;
...
inl::t_u32 TextDocument::size()
{
	return m_nDocLength_bytes;
}

TextIterator TextDocument::iterate(inl::t_u32 offset_chars)
{
	inl::t_u32 off_bytes = charoffset_to_byteoffset(offset_chars);
	inl::t_u32 len_bytes = m_nDocLength_bytes - off_bytes;
...
//
//
TextIterator TextDocument::iterate_line(inl::t_u32 lineno, inl::t_u32 *linestart, inl::t_u32 *linelen)
{
	inl::t_u32 offset_bytes;
	inl::t_u32 length_bytes;
...
TextIterator TextDocument::iterate_line_offset(inl::t_u32 offset_chars, inl::t_u32 *lineno, inl::t_u32 *linestart)
{
	inl::t_u32 offset_bytes;
	inl::t_u32 length_bytes;
...
inl::t_u32 TextDocument::lineno_from_offset(inl::t_u32 offset)
{
	inl::t_u32 lineno = 0;
...
inl::t_u32 TextDocument::offset_from_lineno(inl::t_u32 lineno)
{
	inl::t_u32 lineoff = 0;
...
//	
inl::t_u32 TextDocument::getline(inl::t_u32 nLineNo, TCHAR *buf, inl::t_u32 buflen, inl::t_u32 *off_chars)
{
	inl::t_u32 offset_bytes;
	inl::t_u32 length_bytes;
	inl::t_u32 offset_chars;
	inl::t_u32 length_chars;
...
size_t TextDocument::rawdata_to_utf8(BYTE *rawdata, size_t rawlen, BYTE *utf8str, size_t *utf8len)
{
    switch(m_nFileFormat)
    {
        // convert from ANSI->UNICODE
    case NCP_ASCII:
        return copy_utf8(rawdata, rawlen, (UTF8 *)utf8str, utf8len);
    ...
    case NCP_UTF16:
        rawlen /= sizeof(UTF16);
        return utf16_to_utf8((UTF16 *)rawdata, rawlen, (UTF8 *)utf8str, utf8len) * sizeof(TCHAR);

//         // need to convert from big-endian to little-endian
//     case NCP_UTF16BE:
//         rawlen /= sizeof(TCHAR);
//         return utf16be_to_utf8((UTF16 *)rawdata, rawlen, (UTF8 *)utf8str, utf8len) * sizeof(TCHAR);
...
//
inl::t_u32 TextDocument::insert_raw(inl::t_u32 offset_bytes, TCHAR *text, inl::t_u32 length)
{
	BYTE  buf[0x100];
	inl::t_u32 buflen;
	inl::t_u32 copied;
	inl::t_u32 rawlen = 0;
	inl::t_u32 offset = offset_bytes+ m_nHeaderSize;
...
inl::t_u32 TextDocument::replace_raw(inl::t_u32 offset_bytes, TCHAR *text, inl::t_u32 length, inl::t_u32 erase_chars)
{
	BYTE  buf[0x100];
	inl::t_u32 buflen;
	inl::t_u32 copied;
	...
	inl::t_u32 erase_bytes = count_chars(offset_bytes, erase_chars);
...
//
inl::t_u32 TextDocument::erase_raw(inl::t_u32 offset_bytes, inl::t_u32 length)
{
	/*TCHAR  buf[0x100];
	inl::t_u32  buflen;
	inl::t_u32  bytes;
	inl::t_u32  erase_bytes  = 0;
	inl::t_u32  erase_offset = offset_bytes;
	...
	inl::t_u32 erase_bytes  = count_chars(offset_bytes, length);
...
//
inl::t_u32 TextDocument::count_chars(inl::t_u32 offset_bytes, inl::t_u32 length_chars)
{
	switch(m_nFileFormat)
	{
	case NCP_ASCII:
	...
	inl::t_u32 offset_start = offset_bytes;

	while(length_chars && offset_bytes < m_nDocLength_bytes)
	{
		TCHAR buf[0x100];
		inl::t_u32 charlen = inl::Min<inl::t_u32>(length_chars, 0x100);
		inl::t_u32 bytelen;
...
inl::t_u32 TextDocument::byteoffset_to_charoffset(inl::t_u32 offset_bytes)
{
	switch(m_nFileFormat)
	{
	case NCP_ASCII:
...
inl::t_u32 TextDocument::charoffset_to_byteoffset(inl::t_u32 offset_chars)
{
	switch(m_nFileFormat)
	{
	case NCP_ASCII:
	...
	inl::t_u32 lineoff_chars;
	inl::t_u32 lineoff_bytes;
...
//
inl::t_u32 TextDocument::insert_text(inl::t_u32 offset_chars, TCHAR *text, inl::t_u32 length)
{
	inl::t_u32 offset_bytes = charoffset_to_byteoffset(offset_chars);
...
//
inl::t_u32 TextDocument::replace_text(inl::t_u32 offset_chars, TCHAR *text, inl::t_u32 length, inl::t_u32 erase_len)
{
	inl::t_u32 offset_bytes = charoffset_to_byteoffset(offset_chars);
...
//
inl::t_u32 TextDocument::erase_text(inl::t_u32 offset_chars, inl::t_u32 length)
{
	inl::t_u32 offset_bytes = charoffset_to_byteoffset(offset_chars);
	return erase_raw(offset_bytes, length);
}

bool TextDocument::Undo(inl::t_u32 *offset_start, inl::t_u32 *offset_end)
{
	inl::t_u32 start, length;
...
bool TextDocument::Redo(inl::t_u32 *offset_start, inl::t_u32 *offset_end)
{
	inl::t_u32 start, length;
