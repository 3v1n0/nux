...
extern "C" t_color MixRGB(t_color, t_color);
...
//
void TextView::PaintLine(GraphicsContext& GfxContext, inl::t_u32 nLineNo)
{
	Geometry  rect;
...
// 	rect.bottom		= (long)(rect.top + m_nLineHeight);
    ...
    else
    {
        if(m_ColorQuadBuffer0.Size() - 2*m_NumVBColorQuad0*16 < 2*16)
        {
            ...
            m_ColorQuadBuffer0.Add(2*16);
        }
        RenderColorQuadToBuffer(
            m_ColorQuadBuffer0.PtrData() + 2*m_NumVBColorQuad0*16, m_ColorQuadBuffer0.Size() - 2*m_NumVBColorQuad0*16,
            rect, GetTextBkColor());
        ++m_NumVBColorQuad0;
    }
    ...
    GfxContext.PushClippingRectangle(Geometry(m_ViewX + LeftMarginWidth(), rect.GetY(), m_ViewWidth - LeftMarginWidth(), m_ViewHeight));
...
//
void TextView::PaintMargin(GraphicsContext& GfxContext, inl::t_u32 nLineNo, Geometry& margin)
{
	...
	//int nummaxwidth = 60;
	int curx = margin.GetX();
	int cury = margin.GetY() + margin.GetWidth();
...
// 
// 		imgX = rect.GetX() + (selwidth - imgWidth) / 2;
// 		imgY = rect.GetY()  + (margin.GetHeight() - imgHeight) / 2;
// 	}

    if(!b_RenderToVertexBuffer)
        gPainter.Paint2DQuadColor(GfxContext, rect, inl::Color(0xFF222222));
    else
    {
        if(m_ColorQuadBuffer1.Size() - 2*m_NumVBColorQuad1*16 < 2*16)
        {
            // Enlarge the array and preserve the data that it contains.
            m_ColorQuadBuffer1.Add(2*16);
        }
        RenderColorQuadToBuffer(
            m_ColorQuadBuffer1.PtrData() + 2*m_NumVBColorQuad1*16, m_ColorQuadBuffer1.Size() - 2*m_NumVBColorQuad1*16,
            rect, inl::Color(0xFF222222));
        ++m_NumVBColorQuad1;
    }
	...
	if(CheckStyle(TXS_LINENUMBERS))
	{
        ...
        if(!b_RenderToVertexBuffer)
        {
            gPainter.Paint2DQuadColor(GfxContext, rect, inl::Color(0xFF111111));
            gPainter.PaintTextLineStatic(GfxContext, m_TextFont, rect, ach, inl::Color(0xFFFFFFFF), eAlignTextRight);
        }
        else
        {
            int num_char = inl::StringLength(ach);
            if(m_TextVertexBuffer1.GetNumAvailable() < 3*num_char*16)
            {
                // Enlarge the array and preserve the data that it contains.
                m_TextVertexBuffer1.Add(3*num_char*16);
            }
            
            int n = m_TextFontRenderer->RenderTextToBuffer(
                m_TextVertexBuffer1.PtrData() + 3*m_NumVBQuad1*16, m_TextVertexBuffer1.Size() - 3*m_NumVBQuad1*16,
                m_TextFont, rect, ach, inl::Color(0xFFFFFFFF), eAlignTextRight);
            m_NumVBQuad1 += n;
...
//
void TextView::PaintText(GraphicsContext& GfxContext, inl::t_u32 nLineNo, Geometry& rect)
{
...
// //
// int TextView::ApplyTextAttributes(inl::t_u32 nLineNo, inl::t_u32 nOffset, inl::t_u32 &nColumn, TCHAR *szText, int nTextLen, ATTR *attr)
// {
// 	int	font	= nLineNo % m_nNumFonts;
// 	int i;
// 
//     inl::t_u32 selstart = std::min(m_nSelectionStart, m_nSelectionEnd);
//     inl::t_u32 selend   = std::max(m_nSelectionStart, m_nSelectionEnd);
// 
// 	//
...
// 		if(nColumn >= (inl::t_u32)m_nLongLineLimit && CheckStyle(TXS_LONGLINES))
// 		{
...
int TextView::NeatTextOut(GraphicsContext& GfxContext, int xpos, int ypos, TCHAR *szText, int nLen, int nTabOrigin, ATTR *attr)
{
	...
	for(i = 0; i <= nLen; )
	{
		...
		if(i == nLen || szText[i] == '\t' || (TBYTE)szText[i] < 32)
		{
            ...
            if(!b_RenderToVertexBuffer)
            {
                gPainter.Paint2DQuadColor(GfxContext, rect, inl::Color(attr->bg));
                gPainter.PaintTextLineStatic(GfxContext, m_TextFont, rect, text.GetTCharPtr(), GetTextColor());
            }
            else
            {
                int num_char = text.Size();
                if(m_TextVertexBuffer0.GetNumAvailable() < 3*num_char*16)
                {
                    // Enlarge the array and preserve the data that it contains.
                    m_TextVertexBuffer0.Add(3*num_char*16);
                }
                int n = m_TextFontRenderer->RenderTextToBuffer(
                    m_TextVertexBuffer0.PtrData() + 3*m_NumVBQuad0*16, m_TextVertexBuffer0.Size() - 3*m_NumVBQuad0*16,
...
void TextView::PaintRect(GraphicsContext& GfxContext, int x, int y, int width, int height, t_color fill)
{
...
//
//
t_color TextView::LineColour(inl::t_u32 nLineNo)
{
...
t_color TextView::LongColour(inl::t_u32 nLineNo)
{
	if(m_nCurrentLine == nLineNo && CheckStyle(TXS_HIGHLIGHTCURLINE))
		return m_rgbColourList[TXC_CURRENTLINE];
	else
...
t_color MixRGB(t_color rgbCol1, t_color rgbCol2)
{
	return RGB(
...
//
t_color TextView::GetColour(UINT idx)
{
...
t_color TextView::SetColour(UINT idx, t_color rgbColour)
{
	t_color rgbOld;
...
//
// void TextView::DrawCheckedRect(GraphicsContext& GfxContext, RECT *rect, t_color fg, t_color bg)
// {
...
void TextView::RenderColorQuadToBuffer(float* VertexBuffer, int VBSize, inl::Rect geo, inl::Color color)
{
    inl::Vector4* Position = (inl::Vector4*)VertexBuffer;

    {

        // Position upper left
        //                            Scale       Offset
        Position[0].x = 0.0f  * geo.GetWidth()     + geo.GetX();    // x
        Position[0].y = 0.0f  * geo.GetHeight()    + geo.GetY();           // y
        ...
        // Position lower left
        Position[2].x = 0.0f  * geo.GetWidth()     + geo.GetX();    // x
        Position[2].y = 1.0f  * geo.GetHeight()    + geo.GetY();           // y
        ...
        Position[4].x = 1.0f  * geo.GetWidth()     + geo.GetX();    // x
        Position[4].y = 1.0f  * geo.GetHeight()    + geo.GetY();           // y
        ...
        Position[6].x = 1.0f  * geo.GetWidth()     + geo.GetX();    // x
        Position[6].y = 0.0f  * geo.GetHeight()    + geo.GetY();           // y
