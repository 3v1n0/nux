...
public:
    ...
    NThreadSafeCounter(t_u32 i) {m_Counter = i;}
    t_u32 Increment();
    t_u32 Decrement();
    t_u32 Set(t_u32 i);
    t_u32 GetValue() const;
    t_u32 operator ++ ();
    t_u32 operator -- ();
    t_bool operator == (t_u32 i);
private:
    t_u32 m_Counter;
};
...
public:
    ...
    /*!
        Initialize critical section.
    */
    NCriticalSection() { sys_mutex_create(&m_lock, &mut_attr); }

    //! Destroy critical section.
    /*!
        Destroy critical section.
    */
    ~NCriticalSection() { sys_mutex_destroy(m_lock); }
...
inline		type GetTLS_##name() { return NThreadLocalStorage::GetData<type>(ThreadLocalStorageDef##name::Index); }\
inline		void SetTLS_##name(type value) { NThreadLocalStorage::SetData<type>(ThreadLocalStorageDef##name::Index, value); }

#define inlRegisterThreadLocalIndex(index, name, shutdownCallback) \
    nuxVerifyExpr(index == ThreadLocalStorageDef##name::Index); \
    nuxVerifyExpr(NThreadLocalStorage::RegisterTLS(index, shutdownCallback)) 
...
typedef void (*ThreadRoutineFunc)(t_u64);
...
protected:
    ...
    */
    static void EntryPoint(t_u64 pArg);
...
private:
    ...
    */
    ...
    public:
        ...
        sys_ppu_thread_t  m_dwTID;  //	The Thread ID
